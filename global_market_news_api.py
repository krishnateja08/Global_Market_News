...
def fetch_rss(url: str) -> list:
    ...
    try:
        ...
        source_map = {
            ...
            "moneycontrol.com": "MoneyControl", "federalreserve.gov": "Federal Reserve",
        }
...
def fetch_category_news(category: str, urls: list) -> list:
    seen_titles = set()
    results = []
...
def generate_news_items_html(items: list, cat_id: str) -> str:
    if not items:
...
def generate_complete_html(all_news: dict) -> str:
    ...
    # NOTE: All {{ }} in the JS/CSS below are Python f-string escapes for literal { }
...
// ── FIX: 'usdinr' added WITH comma after 'silver' ──
const SYMBOLS = {{
    'dow':    '^DJI',
    'sp500':  '^GSPC',
    'nasdaq': '^IXIC',
...
// ── FIX: corsproxy.io removed, allorigins.win is primary proxy ──
...
async function fetchGiftNifty() {{
    ...
    try {{
        const r = await fetch('https://api.allorigins.win/get?url='+encodeURIComponent(nseUrl), {{signal: AbortSignal.timeout(6000)}});
        if (r.ok) {{
            let d = await r.json();
            if (d.contents) d = JSON.parse(d.contents);
            if (d && d.length > 0) {{
                const cur = parseFloat(d[d.length-1][1]);
                const prev = parseFloat(d[0][1]);
                if (cur && prev) {{
                    const chg = (cur-prev).toFixed(2);
                    const pct = (((cur-prev)/prev)*100).toFixed(2);
                    ...
                    if (vEl) vEl.textContent = cur.toLocaleString('en-US',{{minimumFractionDigits:2,maximumFractionDigits:2}});
                    if (cEl) {{
    ...
    // Fallback: Nifty 50 spot via Yahoo
    try {{
...
// ── FIX: CPI uses allorigins.win proxy instead of corsproxy.io ──
async function fetchCPI() {{
    const csvUrl = 'https://fred.stlouisfed.org/graph/fredgraph.csv?id=CPIAUCSL';
    const proxiedCsv = 'https://api.allorigins.win/get?url='+encodeURIComponent(csvUrl);
    try {{
...
function tick() {{
    ...
    if (el) el.textContent = '📅 Page generated: {current_time} | Browser: '+now.toLocaleString('en-US',opts)+' IST';
}}
